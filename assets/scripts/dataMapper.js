...
const normalizeVehicle = (row, idx = 0, { getField: fieldGetter, toStateCode, resolveCoords } = {}) => {
  ...
  const v = {
    ...
    movementStatusV2: field(row, 'movement_status_v2'),
    movementDaysStationaryV2: field(row, 'movement_days_stationary_v2'),
    movementThresholdMetersV2: field(row, 'movement_threshold_meters_v2'),
    movementUnitTypeV2: field(row, 'movement_unit_type_v2'),
    movingCalc: field(row, 'Moving (Calc)', 'moving_calc'),
    ptStatus: field(row, 'PT Status', 'pt_status'),
    ptSerial: field(row, 'PT Serial ', 'PT Serial', 'pt_serial', 'pt serial'),
    winnerSerial: field(
      row,
      'Winner Serial',
      'winner_serial',
